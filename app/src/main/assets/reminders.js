...
.rem-head,.setting-line{display:flex;align-items:center;justify-content:space-between;gap:12px}.rem-title{font-size:13px;font-weight:700}
...
.lesson-favorite{position:absolute!important;right:12px!important;top:12px!important;width:38px!important;height:38px!important;margin:0!important;padding:0!important;border-radius:12px!important;background:rgba(255,255,255,.045)!important;border:1px solid rgba(255,255,255,.04)!important;color:#77707f!important;font-size:22px!important;line-height:36px!important;text-align:center!important;display:flex!important;align-items:center!important;justify-content:center!important;z-index:5!important;transition:transform .16s ease,background .16s ease,color .16s ease,box-shadow .16s ease!important}
.lesson-favorite:active{transform:scale(.88)!important}.lesson-favorite.on{color:#ffd86a!important;background:rgba(255,216,106,.09)!important;border-color:rgba(255,216,106,.2)!important;text-shadow:0 0 8px rgba(255,216,106,.75),0 0 16px rgba(255,216,106,.3)!important;box-shadow:0 0 14px rgba(255,216,106,.08)!important}
.lesson-note-wrap{margin-top:9px;display:flex;align-items:center;gap:7px;max-width:100%;box-sizing:border-box}.lesson-note-btn{display:inline-flex;align-items:center;justify-content:center;min-height:32px;margin:0!important;padding:6px 10px!important;border-radius:10px!important;background:rgba(255,255,255,.045)!important;border:1px solid rgba(255,255,255,.05)!important;color:#9b929f!important;font-size:11px!important}.lesson-note-btn.has-note{color:var(--accent)!important;background:var(--accentSoft)!important;border-color:var(--accentBorder)!important}.lesson-note-text{flex:1;min-width:0;color:#aaa1af;font-size:11px;line-height:1.35;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;padding:7px 9px;border-radius:10px;background:rgba(255,255,255,.035)}
...
function renderAuthor(){
  ...
  box.innerHTML=`<h3>👨‍💻 Автор</h3><div class="author-row"><div class="author-avatar">T</div><div class="author-info"><div class="author-name">Takusima</div><div class="author-line">Симаченко Станислав . Р<br>Группа КМК 10 м/с</div><div class="author-tg">Telegram: @takusima</div></div></div>`;
}
...
function ensureNoteModal(){
  ...
  modal.querySelector('#noteSave').onclick=()=>{const key=modal.dataset.key;const value=modal.querySelector('#noteText').value.trim();if(value)notes[key]=value;else delete notes[key];saveNotes();modal.classList.remove('open');decorateLessons();};
  modal.querySelector('#noteDelete').onclick=()=>{const key=modal.dataset.key;delete notes[key];saveNotes();modal.classList.remove('open');decorateLessons();};
  return modal;
}
...
function openNote(key,title,subtitle){
  ...
  modal.querySelector('#noteTitle').textContent=`Заметка • ${title}`;
  modal.querySelector('#noteSubtitle').textContent=subtitle||'Личная заметка к паре';
...
function decorateLessons(){
  document.querySelectorAll('.lesson').forEach(card=>{
    ...
    if(!fav){
      ...
      fav.textContent=favorites.includes(d.name)?'★':'☆';
      fav.onclick=e=>{e.preventDefault();e.stopPropagation();const subject=fav.dataset.subject;if(favorites.includes(subject))favorites=favorites.filter(x=>x!==subject);else favorites.push(subject);saveFav();updateFavoriteButtons();renderFavorites();};
      card.appendChild(fav);
    }else fav.dataset.subject=d.name;

    let wrap=card.querySelector('.lesson-note-wrap');
    if(!wrap){
      wrap=document.createElement('div');wrap.className='lesson-note-wrap';
      const btn=document.createElement('button');btn.className='lesson-note-btn';btn.type='button';
      btn.onclick=e=>{e.preventDefault();e.stopPropagation();openNote(key,d.name,`${d.time||''}${d.room?' • Кабинет: '+d.room:''}`)};
      wrap.appendChild(btn);
      const room=card.querySelector('.lesson-room');
      if(room&&room.parentElement===card)room.insertAdjacentElement('afterend',wrap);else card.appendChild(wrap);
    }
    const btn=wrap.querySelector('.lesson-note-btn');
    const text=notes[key]||'';
    btn.textContent=text?'📝 Изменить заметку':'📝 Добавить заметку';
    btn.classList.toggle('has-note',!!text);
    let preview=wrap.querySelector('.lesson-note-text');
    if(text){
      if(!preview){preview=document.createElement('div');preview.className='lesson-note-text';wrap.insertBefore(preview,btn)}
...
function boot(){
  ...
  document.addEventListener('click',e=>{
    const b=e.target.closest?.('.lesson-favorite');
