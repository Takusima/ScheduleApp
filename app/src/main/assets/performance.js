...
function finishApply() {
  if (typeof state === 'undefined') { failExcel('Состояние приложения ещё не готово.'); return; }
  const files = window.__scheduleFilesForApply || [];
  state.files = files.map(x => ({name: x.name, data: x.data}));
  state.data = {};
  const seen = new Map();
  ...
  function processChunk() {
    ...
    for (; index < end; index++) {
      ...
      if (!groupSeen) { groupSeen = new Set(); seen.set(key, groupSeen); }
    ...
    if (index < queue.length) { requestAnimationFrame(processChunk); return; }

    state.groups = Array.from(new Set(Object.values(state.data).flatMap(dateGroups => Object.keys(dateGroups))))
      .sort((a,b) => a.localeCompare(b, 'ru', {numeric:true}));
    state.dates = Object.keys(state.data).sort();
    state.dates.forEach(date => Object.keys(state.data[date]).forEach(group => {
      state.data[date][group].sort((a,b) => timeToMinutesFast(a.time) - timeToMinutesFast(b.time));
    }));

    if (!state.selectedGroup || !state.groups.includes(state.selectedGroup)) state.selectedGroup = state.groups[0] || '';
    ...
    updateGroupSelect(); updateGroupSelectValue(); updateDateSelect(); updateDateSelectValue();
    updateDays(); updateSchedule(); updateCalendar(); updateSettingsInfo();
    workerBusy = false;
    const status = pendingStatus;
    const notify = pendingNotify;
    pendingFiles = []; pendingLessons = []; pendingStatus = ''; pendingNotify = false;
    ...
    } catch(e) { console.error('Ошибка обновления напоминаний:', e); }
...
function ensureWorker() {
  ...
  try {
    ...
    excelWorker.onmessage = event => {
      ...
      if (!result.ok) { workerBusy=false; failExcel(result.error || 'Не удалось разобрать Excel.'); return; }
      pendingLessons.push(...(result.lessons || []));
      pendingFiles.shift();
      if (pendingFiles.length) { processNextFile(); return; }
      finishApply();
    };
    excelWorker.onerror = event => {
      console.error('Excel Worker error:', event);
      try { excelWorker.terminate(); } catch(_) {}
      excelWorker = null; workerBusy=false;
      failExcel('Фоновый обработчик Excel завершился с ошибкой.');
    };
    return excelWorker;
  } catch(error) { console.error('Worker creation error:', error); excelWorker=null; return null; }
}
function processNextFile() {
  const worker = ensureWorker();
  if (!worker) { workerBusy=false; if(typeof originalOnNativeFiles==='function') originalOnNativeFiles(JSON.stringify(pendingFiles), pendingStatus); return; }
  const file = pendingFiles[0];
  if (!file) { finishApply(); return; }
  if (typeof setStatus === 'function') setStatus('Обработка Excel в фоне...');
  try { worker.postMessage({name:file.name,data:file.data}); }
  catch(error) { failExcel(error.message || 'Не удалось передать Excel в фоновый обработчик.'); }
}
window.onNativeFiles = function(json, status) {
  ...
  } catch(error) { failExcel(error.message || 'Неверный формат Excel-файлов'); return; }
