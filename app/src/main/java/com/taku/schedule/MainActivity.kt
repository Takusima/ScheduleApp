...
class MainActivity : AppCompatActivity() {
    ...
    inner class Bridge {
        ...
        fun sync() {
            thread {
                try {
                    ...
                    val folder = File(cacheDir, "schedule_cache").apply { mkdirs() }
                    ...
                    files.forEach { file ->
                        ...
                        item.put("data", android.util.Base64.encodeToString(bytes, android.util.Base64.NO_WRAP))
                        result.put(item)
                    }
                    runOnUiThread {
                        web.evaluateJavascript("window.onNativeFiles(${JSONObject.quote(result.toString())});", null)
                    }
                } catch (e: Exception) {
                    runOnUiThread {
                        web.evaluateJavascript("window.onNativeError(${JSONObject.quote(e.message ?: "Ошибка обновления")});", null)
                    }
                }
            }
        }
        ...
        fun openSource() {
            runOnUiThread {
                android.content.Intent(android.content.Intent.ACTION_VIEW, URL("https://cloud.mail.ru/public/$publicCode").toURI())
    ...
    private fun createZipLink(code: String): String {
        ...
        if (conn.responseCode !in 200..299) error("Mail Cloud: HTTP ${conn.responseCode}")
        val obj = JSONObject(text)
        val key = obj.optString("key")
        if (key.isBlank()) error("Mail Cloud не вернул ссылку на архив")
        ...
        ZipInputStream(zipBytes.inputStream()).use { zis ->
            while (true) {
                ...
                if (!entry.isDirectory && entry.name.lowercase().endsWith(".xlsx")) {
                    val safeName = entry.name.substringAfterLast('/').replace(Regex("[^\\p{L}\\p{N}._ -]"), "_")
